...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--residual_head", action="store_true",
                   help="Predict the DEVIATION from the last observation "
    ...
    p.add_argument("--value_embedding",     type=str, default="linear",
                   ...
                   help="Observation encoder. 'linear' (the default) is "
    ...
    p.add_argument("--static_in_token",     action="store_true",
                   help="Aurora-style: put the 15 static station features "
...
def main() -> None:
    ...
    if args.value_embedding != "linear":
        print(f"Observation encoder      : {args.value_embedding}")
    if args.static_in_token:
        print("Static features          : INSIDE the variable block — "
    ...
    cfg = {
        ...
        "readout":             args.readout,
        "temporal_patch":      args.temporal_patch,
        # Structural: MUST be recorded or evaluation rebuilds the
