...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output_mode", type=str, default="grid", choices=["grid", "dense"],
                   help="grid  (A): head width = delta_grid_stride grid, K=13 at stride 3. "
                        "Matches the transformer exactly -> paired comparison. "
                        "dense (C): forces stride 1, K=37 at 10-min resolution; ALL 37 "
                        "supervised, the 13 comparison horizons selected at eval time.")
...
def main() -> None:
    ...
    # ── Output-grid mode ─────────────────────────────────────────────────
    # A "grid":  supervise the 13 horizons the transformer predicts (stride 3).
    # C "dense": supervise all 37 10-min horizons (stride 1); the 13 comparison
    #            horizons are selected at evaluation time (test_lstm.py).
    # Both supervise EVERY output slot — a head wider than the loss would leave
    # slots with no gradient, so that combination is deliberately unavailable.
    _stride = 1 if args.output_mode == "dense" else args.delta_grid_stride
    if args.output_mode == "dense" and args.delta_grid_stride != 1:
        print(f"  output_mode=dense -> overriding delta_grid_stride "
              f"{args.delta_grid_stride} -> 1 (10-min targets)")
    ...
    common = dict(window_size=args.window, delta_steps=args.max_delta,
                  ...
                  delta_mode="fixed_grid", delta_grid_stride=_stride)
    ...
    print(f"  output_mode={args.output_mode} | Δ (10-min steps) = {train_ds.delta_grid}")
    print(f"  every one of the {K} slots is supervised by the loss")
    ...
    model = StationLSTM(num_vars=NUM_VARIABLES, num_target_vars=NUM_TARGET_VARIABLES,
                        ...
                        horizon_steps=train_ds.delta_grid,
    ...
    print(f"  head: Linear({args.hidden} -> {K * NUM_TARGET_VARIABLES})  "
          f"= {K} horizons x {NUM_TARGET_VARIABLES} target vars")
    ...
    cfg = dict(lr=args.lr, min_lr=args.min_lr, weight_decay=args.weight_decay,
               ...
               max_delta=args.max_delta, delta_grid_stride=_stride,
               output_mode=args.output_mode, horizon_steps=list(train_ds.delta_grid),
